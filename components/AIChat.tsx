...
const AIChat: React.FC<AIChatProps> = ({ config }) => {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    { role: 'assistant', content: '¡Hola! Soy el chat inteligente de Bodipo. Pregúntame sobre tarifas, horarios, o servicios y te responderé con la información de la web.' }
  ]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'assistant', content: 'Nuestros sistemas están procesando envíos de alta prioridad. Por favor, reintenta en unos instantes.' }]);
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div id="ayuda" className="fixed bottom-10 right-10 z-50">
      {!isOpen ? (
        ...
        >
          ...
          <span className="ml-4 font-black uppercase text-[10px] tracking-[0.3em] hidden md:inline">Chat de Ayuda</span>
        </button>
      ) : (
        <div className="bg-white w-[400px] h-[600px] rounded-[3rem] shadow-[0_50px_100px_rgba(0,0,0,0.2)] flex flex-col overflow-hidden border border-gray-100 transition-all animate-in slide-in-from-bottom-20">
          <div className="bg-[#00151a] p-8 text-white flex justify-between items-center">
            <div className="flex items-center gap-5">
              <div className="w-12 h-12 bg-white rounded-2xl flex items-center justify-center border-[4px] border-white shadow-sm overflow-hidden p-1">
                {config?.customLogoUrl ? (
                  ...
                  <span className="logo-font text-3xl logo-color leading-none select-none pt-1">{config?.logoText || 'bb'}</span>
                )}
              </div>
              <div>
                <p className="font-black text-xs uppercase tracking-widest leading-none mb-1">Bodipo Business</p>
            ...
            <button onClick={() => setIsOpen(false)} className="hover:bg-white/10 rounded-full p-2.5 transition-colors">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" /></svg>
            </button>
          </div>

          <div ref={scrollRef} className="flex-1 overflow-y-auto p-8 space-y-6 bg-[#fcfdfd]">
            {messages.map((msg, idx) => (
              <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[85%] px-6 py-4 rounded-3xl text-sm leading-relaxed font-medium ${msg.role === 'user'
          ...
          <div className="p-8 border-t border-gray-50 bg-white">
            <div className="flex gap-4">
              <input
                ...
                className="flex-1 px-6 py-4 bg-gray-50 border-none rounded-2xl text-sm font-medium focus:outline-none focus:ring-2 focus:ring-[#007e85] transition-all"
                value={input}
            ...
            <p className="text-[8px] text-gray-300 mt-6 text-center font-black uppercase tracking-[0.4em]">Bodipo Intelligence System</p>
          </div>
        </div>
      )}
    </div>
