...
const Header: React.FC<HeaderProps> = ({ onOpenRegister, onOpenLogin, config }) => {
  ...
  const navItems = [
    ...
    { path: '/tarifas', label: 'Envíos' },
    { path: '/money-transfer', label: 'Money Transfer' },
    { path: '/tienda', label: 'Tienda' },
    { path: '/rastreo', label: 'Rastrea tu paquete' },
                ...
                {isAuthenticated && user ? (
                  <div className="relative">
                    <button
                      ...
                      className="flex items-center space-x-2 bg-[#00151a] text-white px-6 py-2.5 rounded-full hover:bg-[#007e85] transition-all shadow-lg shadow-teal-900/20"
                    >
                      <span>{user.name}</span>
                    ...
                    {showUserMenu && (
                      <div className="absolute right-0 mt-2 w-48 bg-white rounded-2xl shadow-xl border border-gray-100 py-2 normal-case tracking-normal text-sm font-medium">
                        <Link
                          to="/dashboard"
