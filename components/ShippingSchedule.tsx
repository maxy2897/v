...
const ShippingSchedule: React.FC = () => {
  // Ajustado a la fecha actual indicada por el usuario: 5 de Enero 2026
  const currentMonth = "Enero 2026";

  // Próximas salidas confirmadas para Enero 2026 según el calendario oficial
  const currentShipments = [
    { day: '17', type: 'Aéreo', status: 'Abierto', destination: 'Malabo / Bata', highlight: true },
    { day: '30', type: 'Aéreo', status: 'Abierto', destination: 'Malabo / Bata' },
  ];
  ...
  return (
    <section id="calendario" className="py-24 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
          <div className="max-w-2xl">
            <div className="inline-flex items-center space-x-2 bg-teal-50 px-3 py-1 rounded-full mb-4">
              ...
              <span className="text-[10px] font-black uppercase tracking-widest text-teal-900">Estado: Operativo 2026</span>
            </div>
            <h2 className="text-4xl md:text-5xl font-black text-[#00151a] tracking-tighter">
              Calendario de <br /><span className="text-[#005f6b]">Expediciones</span>
            </h2>
          </div>
          <div className="bg-white px-6 py-3 rounded-2xl shadow-sm border border-gray-100">
            <p className="text-xs font-black text-gray-400 uppercase tracking-[0.2em] mb-1">Mes en curso</p>
        ...
        {/* Salidas del Mes Actual (Enero) */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
          {currentShipments.map((ship, idx) => (
            <div
              key={idx}
            ...
            >
              ...
              <div className="flex justify-between items-start mb-6">
                <div className="flex flex-col">
                  ...
                  <span className="text-xs font-bold uppercase tracking-widest opacity-60">Enero</span>
                </div>
              ...
              <div className="space-y-4">
                <div>
                  <p className={`text-[10px] font-black uppercase tracking-widest mb-1 ${ship.highlight ? 'text-teal-400' : 'text-gray-400'}`}>Tipo de Servicio</p>
                  <p className="font-bold text-lg">{ship.type}</p>
                </div>
                <div>
                  <p className={`text-[10px] font-black uppercase tracking-widest mb-1 ${ship.highlight ? 'text-teal-400' : 'text-gray-400'}`}>Destino</p>
          ...
          <div className="p-8 rounded-[2.5rem] border border-dashed border-teal-200 bg-white flex flex-col justify-center items-center text-center shadow-sm">
            ...
            <p className="text-xs font-black uppercase tracking-widest text-teal-800">Documentos G.E. {'->'} ES</p>
            <p className="text-[10px] font-bold text-gray-500 mt-2 uppercase italic">Salidas semanales. Consultar anuncio en WhatsApp.</p>
            <span className="mt-4 bg-[#00151a] text-white px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest">15€ Tarifa Fija</span>
          </div>
        </div>
          ...
          <div className="flex items-start">
            ...
            <div className="ml-4">
              <p className="text-sm font-black text-amber-800 uppercase tracking-widest">
                Aviso Importante para Envíos
              </p>
              <p className="mt-1 text-sm text-amber-700 font-bold leading-relaxed">
                Los paquetes deben estar en nuestros almacenes <span className="underline decoration-2 underline-offset-2">a más tardar el día anterior a la salida a las 15:00</span> para garantizar su embarque.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-[#f8fcfc] rounded-[3rem] p-8 md:p-12 border border-teal-100 shadow-sm">
          <div className="flex items-center space-x-4 mb-10">
            ...
            <h3 className="text-2xl font-black text-[#00151a] tracking-tight">Programación 2026 - Vista General</h3>
          </div>
            ...
            {yearlyOverview.map((item, i) => (
              <div key={i} className={`p-6 rounded-3xl border shadow-sm transition-all ${item.month === 'ENERO' ? 'bg-white border-teal-500 ring-2 ring-teal-500/10 scale-105' : 'bg-white/50 border-teal-50 grayscale-[0.5]'}`}>
                ...
                <div className="mt-4 pt-4 border-t border-gray-50 flex items-center text-[9px] font-bold text-gray-400 uppercase tracking-widest">
                  ...
                  {item.month === 'ENERO' ? 'Mes Actual' : 'Programación'}
                </div>
              </div>
            ))}
          </div>
