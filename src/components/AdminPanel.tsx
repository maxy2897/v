            ...
            return (
              <div key={req._id} className="p-8 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-10 hover:bg-gray-50/30 transition-all">
                   ...
                   <div className="relative group cursor-pointer" onClick={() => setViewingCapture(captureUrl)}>
                      {captureUrl ? (
                        <div className="h-24 w-full rounded-2xl overflow-hidden border-2 border-dashed border-gray-200 group-hover:border-teal-400 transition-all bg-gray-50 flex items-center justify-center relative">
                           ...
                           <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-all"><span className="text-[9px] font-black text-white uppercase tracking-widest">Ver Captura</span></div>
                        </div>
                      ) : (
                ...
                <div className="flex items-center gap-3 shrink-0">
                  <button onClick={() => handleProcessActivation(req)} disabled={isSavingThis} className="px-8 py-4 bg-teal-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-teal-500 transition-all disabled:opacity-50 shadow-lg shadow-teal-500/20 active:scale-95">
                      ...
                      <div className="space-y-6">
                         <div className="p-6 bg-white/5 rounded-3xl border border-white/10">
                            ...
                            <div className="space-y-4">
                               ...
                               <div className="flex items-center justify-between">
                                  <span className="text-xs font-bold text-gray-400 tracking-widest uppercase">{t('admin.op_rate_air_es_bata')}</span>
                                  <input type="number" placeholder="11" className="bg-white/10 border border-white/20 rounded-xl px-4 py-2 w-24 text-center font-black" value={editConfig?.rates?.air?.es_bata || ''} onChange={e => {
                                      const num = parseFloat(e.target.value) || 0;
                                      setEditConfig({ ...editConfig, rates: { ...editConfig?.rates, air: { ...editConfig?.rates?.air, es_bata: num } }, starRates: { ...editConfig?.starRates, air_es_bata: num + "€/Kg" } } as any);
                                   }} />
                               </div>
                               <div className="flex items-center justify-between">
                               ...
                               <div className="flex items-center justify-between">
                                  <span className="text-xs font-bold text-gray-400 tracking-widest uppercase">{t('admin.op_rate_sea_es_bata')}</span>
                                  <input type="number" placeholder="4" className="bg-white/10 border border-white/20 rounded-xl px-4 py-2 w-24 text-center font-black" value={editConfig?.rates?.sea?.es_bata || ''} onChange={e => {
                                      const num = parseFloat(e.target.value) || 0;
                                      setEditConfig({ ...editConfig, rates: { ...editConfig?.rates, sea: { ...editConfig?.rates?.sea, es_bata: num } }, starRates: { ...editConfig?.starRates, sea_es_bata: num + "€/Kg" } } as any);
                                   }} />
                               </div>
                               <div className="flex items-center justify-between">
                               ...
                               <div className="flex items-center justify-between">
                                  <span className="text-xs font-bold text-gray-400 tracking-widest uppercase">{t('admin.op_rate_gq_es')}</span>
                                  <input type="number" placeholder="15" className="bg-white/10 border border-white/20 rounded-xl px-4 py-2 w-24 text-center font-black" value={editConfig?.rates?.air?.gq_es || ''} onChange={e => {
                                      const num = parseFloat(e.target.value) || 0;
                                      setEditConfig({ ...editConfig, rates: { ...editConfig?.rates, air: { ...editConfig?.rates?.air, gq_es: num } }, starRates: { ...editConfig?.starRates, docs_gq_es: num + "€" } } as any);
                                   }} />
                               </div>
                            </div>
                         </div>
