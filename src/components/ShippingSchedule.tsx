...
const ShippingSchedule: React.FC = () => {
  ...
  const parsedNextAir = appConfig?.dates?.nextAirDeparture ? new Date(appConfig.dates.nextAirDeparture) : null;
  const parsedNextSea = appConfig?.dates?.nextSeaDeparture ? new Date(appConfig.dates.nextSeaDeparture) : null;

  const nextAir = parsedNextAir && !isNaN(parsedNextAir.getTime()) ? parsedNextAir : null;
  const nextSea = parsedNextSea && !isNaN(parsedNextSea.getTime()) ? parsedNextSea : null;
  ...
  return (
    <section id="calendario" className="pt-10 pb-24 relative overflow-hidden [clip-path:inset(0)]">
      <div className="fixed inset-0 z-0 bg-[url('/images/bg/calendar-net-v2.png')] bg-cover bg-center blur-[3px] brightness-[1.2] pointer-events-none"></div>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        {/* Panel Centralizado - Ubicado en el "espacio" pero más abajo */}
        <div className="flex justify-center mb-16 mt-4">
           {nextShipment && (
              <div className="bg-teal-50/10 dark:bg-teal-900/10 backdrop-blur-md border border-teal-100/20 rounded-2xl p-5 flex items-center gap-5 shadow-sm relative overflow-hidden w-full max-w-lg">
